...
class TestServerCharacter():
	def test_sets_attribute_values_to_defaults(self):
		character = server_character.ServerCharacter(config.STARTING_CHARACTERS[0])
		expected_speed = config.STARTING_CHARACTERS[0]['speed'][config.STARTING_CHARACTERS[0]['speed_index']]
		expected_might = config.STARTING_CHARACTERS[0]['might'][config.STARTING_CHARACTERS[0]['might_index']]
		expected_sanity = config.STARTING_CHARACTERS[0]['sanity'][config.STARTING_CHARACTERS[0]['sanity_index']]
		expected_knowledge = config.STARTING_CHARACTERS[0]['knowledge'][config.STARTING_CHARACTERS[0]['knowledge_index']]
	...
	def test_allows_changing_attribute_value(self):
		...
		expected_speed = config.STARTING_CHARACTERS[0]['speed'][config.STARTING_CHARACTERS[0]['speed_index'] + 3]
		assert character.get_attribute_value('speed') == expected_speed

	def test_allows_changing_attribute_value_negatively(self):
		character = server_character.ServerCharacter(config.STARTING_CHARACTERS[0])
		character.change_attribute_value('speed', -2)
		expected_speed = config.STARTING_CHARACTERS[0]['speed'][config.STARTING_CHARACTERS[0]['speed_index'] - 2]
		assert character.get_attribute_value('speed') == expected_speed

	def test_allows_max_index_of_8(self):
		character = server_character.ServerCharacter(config.STARTING_CHARACTERS[0])
		character.change_attribute_value('speed', 15)
		expected_speed = config.STARTING_CHARACTERS[0]['speed'][8]
		assert character.get_attribute_value('speed') == expected_speed

	def test_allows_min_index_of_0(self):
		character = server_character.ServerCharacter(config.STARTING_CHARACTERS[0])
		character.change_attribute_value('speed', -15)
		expected_speed = config.STARTING_CHARACTERS[0]['speed'][0]
		assert character.get_attribute_value('speed') == expected_speed
