...
class ServerCore(Lattice2dServerCore):
	def create_player_handler(self, command):
		already_used = next(iter(p for p in self.get_players() if p.name == command.data['player_name']), False)
		if len(command.data['player_name']) < 6:
			...
			self.get_players().append(Player(command.data['player_name'], command.connection))
	...
	def create_game_handler(self, command):
		...
		if len(command.data['game_name']) < 6:
			...
			player = next(p for p in self.get_players() if p.connection == command.connection)
