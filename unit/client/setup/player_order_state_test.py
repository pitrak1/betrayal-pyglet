...
class TestPlayerOrderState():
	def TestConstructor():
		def test_adds_network_get_player_order(self, mocker, get_args):
	...
	def TestContinue():
		def test_sets_waiting_text(self, mocker, get_args):	
			state = player_order_state.PlayerOrderState({}, mocker.stub(), mocker.stub(), 'player_name', 'game_name', True, testing=True)
		...
		def test_adds_network_confirm_player_order(self, mocker, get_args):	
			state = player_order_state.PlayerOrderState({}, mocker.stub(), mocker.stub(), 'player_name', 'game_name', True, testing=True)
			waiting_text = types.SimpleNamespace()
			waiting_text.text = ''
			state.elements = { 'waiting_text': waiting_text }
		...
		def test_does_not_add_network_confirm_player_order_after_first(self, mocker, get_args):	
			state = player_order_state.PlayerOrderState({}, mocker.stub(), mocker.stub(), 'player_name', 'game_name', True, testing=True)
			waiting_text = types.SimpleNamespace()
			waiting_text.text = ''
			state.elements = { 'waiting_text': waiting_text }
	...
	def TestNext():
		def test_sets_state_to_character_selection_state(self, mocker, get_args):
			state = player_order_state.PlayerOrderState({}, mocker.stub(), mocker.stub(), 'player_name', 'game_name', True, testing=True)
			state.next()
