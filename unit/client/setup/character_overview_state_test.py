...
class TestCharacterOverviewState():
	class TestConstructor():
		def test_adds_network_get_character_selections(self, mocker, get_args):
	...
	class TestConfirmCharacters():
		def test_sets_waiting_text_if_current_player(self, mocker, get_args):
			state = character_overview_state.CharacterOverviewState({}, mocker.stub(), mocker.stub(), 'player_name', 'game_name', True, testing=True)
		...
		def test_adds_network_confirm_character_selections_if_current_player(self, mocker, get_args):
			state = character_overview_state.CharacterOverviewState({}, mocker.stub(), mocker.stub(), 'player_name', 'game_name', True, testing=True)
			waiting_text = types.SimpleNamespace()
			waiting_text.text = ''
			state.elements = { 'waiting_text': waiting_text }
		...
		def test_does_not_add_network_confirm_character_selections_if_not_current_player(self, mocker):
			state = character_overview_state.CharacterOverviewState({}, mocker.stub(), mocker.stub(), 'player_name', 'game_name', True, testing=True)
			waiting_text = types.SimpleNamespace()
			waiting_text.text = ''
			state.elements = { 'waiting_text': waiting_text }
